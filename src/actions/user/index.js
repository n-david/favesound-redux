...
export const fetchFollowings = (user, nextHref) => (dispatch, getState) => {
  ...
  const url = getLazyLoadingUsersUrl(
    user,
    nextHref,
    'followings?limit=20&offset=0',
  );
    ...
    .then(data => {
      ...
      dispatch(
        mergeTrackTypesTrack(
          filter(value => value.type === trackTypes.TRACK, typeMap),
        ),
      );
      dispatch(
        mergeTrackTypesRepost(
          filter(value => value.type === trackTypes.TRACK_REPOST, typeMap),
        ),
      );

      const activitiesMap = flow(filter(isTrack), map('origin'))(
        data.collection,
      );
...
export const fetchFavorites = (user, nextHref) => (dispatch, getState) => {
  ...
  const url = getLazyLoadingUsersUrl(
    user,
    nextHref,
    'favorites?linked_partitioning=1&limit=20&offset=0',
  );
  const requestInProcess = getState().request[requestType];
